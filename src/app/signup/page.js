...
export default function Sms() {
    ...
    useEffect(() => {
        fetch('http://localhost:3000/pages/api/forminput')
            .then((res) => res.json())
            .then((data) => {
                setData(data);
            })
    ...
    const handleSendSms = async () => {
        ...
        const response = await fetch('http://localhost:3000/pages/api/sendsms', {
            method: 'POST',
    ...
    const handleVerify = async () => {
        const response = await fetch('http://localhost:3000/pages/api/authenticate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
