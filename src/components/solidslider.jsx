...
const SolidSlider = () => {
  ...
  useEffect(() => {
    fetch('https://izirtuland.ir/pages/api/solidslider')
      .then((res) => res.json())
      .then((data) => {
        setData(data);
        setLoading(false);
  ...
  if (!data) return <Err />;
  if (!da) return <Err />;
const filter1 = da.filter((item) => item.name.includes("مایع") !== item.name)
  return (
    <div className="max-w-screen-xl mx-auto p-4 bg-color1" dir="rtl">
      <div className='text-center'>
        {
          data.map((items) => (
            items.title.map((item) => (
              <h1 className='text-white md:text-xl text-md font-primaryBold stroke-black-500' key={item.id}>
                {item.name}
              </h1>
            ))
          ))
      ...
      <Swiper
        ...
        modules={[Pagination]}
      >
        {filter1.slice(0,9).map((item) =>
            <SwiperSlide key={item.id}>
              <Link href={`${item.link}`} key={item.id}>
                <div className="relative flex flex-col my-6 bg-white shadow-lg border border-gray-300 rounded-lg w-full" key={item.id}>
                  <Image
                    src={item.pic}
                    layout="responsive"
                    height={200}
